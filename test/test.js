...
describe('Runner', function () {
    ...
    let collection;
    beforeEach((done) => {
        MongoClient.connect('mongodb://localhost:27017/cronTest', { promiseLibrary: Q.Promise })
            .then((db) => {
    ...
    it('should not call the "run" method next time, nor concurrent runners while the first one emitting progress', function () {
        ...
        const task = getMockTask();
        task.run = sinon.spy(function () {
            const def = Q.defer();
            const sendProgress = function() {
                def.notify(Runner.PROGRESS_TASK_IN_PROGRESS);
            };
            setTimeout(sendProgress, 400);
            setTimeout(sendProgress, 800);
            setTimeout(sendProgress, 1200);
            setTimeout(sendProgress, 1600);
            setTimeout(sendProgress, 2000);
            setTimeout(sendProgress, 2400);
            setTimeout(sendProgress, 2800);
